...
class EditorModel
{
    ...
    public function getContenidosReportados(){
        $sql = "SELECT r.id as reporte, e.descripcion as estado, c.id as contenido, r.comentarios, u.mail  FROM contenido c LEFT JOIN reportes_editor r ON r.contenido = c.id LEFT JOIN estado e ON r.estado = e.id LEFT JOIN usuarios u ON r.id_contenidista = u.id WHERE c.estado <> 1";
        return $this->database->query($sql);
    }
